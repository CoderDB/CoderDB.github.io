...
> <h3> 6. List Comprehension </h3>

真正感觉 **Haskell** 的魅力就从这里开始了！

数学中有集合的概念，比如实数集（R）、整数集（Z）、有理数集（Q）等等，这些词提起来满满的都是青春啊！肯定不会陌生啦！通过一些规则可以从既有的集合产生一个新的集合。前10个偶数集合在数学里还记得是怎么表示的吗？
$$ A = \{2 * x | x \in N_+, x \leq 10 \} $$ 在 **Haskell** 中，简直跟数学一模一样。让我们来实现它...

{% highlight Haskell %}
ghci>let a = [2 * x | x <- [1..10]]
ghci>a
